    ...
    hooks: {
      ...
      token: function(stream, state) {
        ...
        if (state.tlvCodeActive) {
          ...
          if (tlvTrackStatements) {
            beginStatement = (stream.peek() != " ") && (style === undefined) && !state.tlvInBlockComment && (stream.column() == state.tlvIndentationStyle.length * tlvIndentUnit);
            if (beginStatement) {
          ...
          if (style !== undefined) {
            ...
            if ((prefix in tlvIdentifierStyle) && (mnemonic.length > 0 || stream.eol())) {
